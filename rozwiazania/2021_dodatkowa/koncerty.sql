...
--
-- Host: 127.0.0.1
-- Czas generowania: 22 Kwi 2023, 19:16
...
--
-- Zastąpiona struktura widoku `miasta_z_liczba_koncertow`
-- (Zobacz poniżej rzeczywisty widok)
--
CREATE TABLE `miasta_z_liczba_koncertow` (
`miasto` varchar(20)
,`kod_miasta` varchar(6)
,`wojewodztwo` varchar(19)
,`ile_koncertow` bigint(21)
);

-- --------------------------------------------------------

--
-- Zastąpiona struktura widoku `polaczone_tabele`
-- (Zobacz poniżej rzeczywisty widok)
--
CREATE TABLE `polaczone_tabele` (
`data` date
,`id` int(3)
,`id_zespolu` int(3)
,`kod_miasta` varchar(6)
...
--
-- Zastąpiona struktura widoku `zespoly_koncertujace_w_okresie`
-- (Zobacz poniżej rzeczywisty widok)
--
CREATE TABLE `zespoly_koncertujace_w_okresie` (
`id_zespolu` int(3)
);
...
--
-- Zastąpiona struktura widoku `zespoly_koncertujace_w_tygodniu`
-- (Zobacz poniżej rzeczywisty widok)
--
CREATE TABLE `zespoly_koncertujace_w_tygodniu` (
`nazwa` varchar(21)
,`id_zespolu` int(3)
,`ile_razy_w_powszednie` bigint(21)
);

-- --------------------------------------------------------

--
-- Zastąpiona struktura widoku `zespoly_koncertujace_w_weekendy`
-- (Zobacz poniżej rzeczywisty widok)
--
CREATE TABLE `zespoly_koncertujace_w_weekendy` (
`nazwa` varchar(21)
,`id_zespolu` int(3)
,`ile_razy_w_weekendy` bigint(21)
);

-- --------------------------------------------------------

--
-- Struktura widoku `miasta_z_liczba_koncertow`
--
DROP TABLE IF EXISTS `miasta_z_liczba_koncertow`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `miasta_z_liczba_koncertow`  AS SELECT `polaczone_tabele`.`miasto` AS `miasto`, `polaczone_tabele`.`kod_miasta` AS `kod_miasta`, `polaczone_tabele`.`wojewodztwo` AS `wojewodztwo`, count(`polaczone_tabele`.`id`) AS `ile_koncertow` FROM `polaczone_tabele` GROUP BY `polaczone_tabele`.`miasto``miasto`  ;

-- --------------------------------------------------------

--
-- Struktura widoku `polaczone_tabele`
--
DROP TABLE IF EXISTS `polaczone_tabele`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `polaczone_tabele`  AS SELECT `data` AS `data`, `id` AS `id`, `id_zespolu` AS `id_zespolu`, `kod_miasta` AS `kod_miasta`, `miasta`.`miasto` AS `miasto`, `miasta`.`wojewodztwo` AS `wojewodztwo`, `zespoly`.`liczba_artystow` AS `liczba_artystow`, `zespoly`.`nazwa` AS `nazwa` FROM ((`koncerty` join `zespoly` on(`id_zespolu` = `zespoly`.`id_zespolu`)) join `miasta` on(`kod_miasta` = `miasta`.`kod_miasta`))  ;

-- --------------------------------------------------------

--
-- Struktura widoku `zespoly_koncertujace_w_okresie`
--
DROP TABLE IF EXISTS `zespoly_koncertujace_w_okresie`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `zespoly_koncertujace_w_okresie`  AS SELECT `polaczone_tabele`.`id_zespolu` AS `id_zespolu` FROM `polaczone_tabele` WHERE `polaczone_tabele`.`data` between '2017-07-20' and '2017-07-25''2017-07-25'  ;

-- --------------------------------------------------------

--
-- Struktura widoku `zespoly_koncertujace_w_tygodniu`
--
DROP TABLE IF EXISTS `zespoly_koncertujace_w_tygodniu`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `zespoly_koncertujace_w_tygodniu`  AS SELECT `polaczone_tabele`.`nazwa` AS `nazwa`, `polaczone_tabele`.`id_zespolu` AS `id_zespolu`, count(0) AS `ile_razy_w_powszednie` FROM `polaczone_tabele` WHERE weekday(`polaczone_tabele`.`data`) < 5 GROUP BY `polaczone_tabele`.`id_zespolu``id_zespolu`  ;

-- --------------------------------------------------------

--
-- Struktura widoku `zespoly_koncertujace_w_weekendy`
--
DROP TABLE IF EXISTS `zespoly_koncertujace_w_weekendy`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `zespoly_koncertujace_w_weekendy`  AS SELECT `polaczone_tabele`.`nazwa` AS `nazwa`, `polaczone_tabele`.`id_zespolu` AS `id_zespolu`, count(0) AS `ile_razy_w_weekendy` FROM `polaczone_tabele` WHERE weekday(`polaczone_tabele`.`data`) = 6 OR weekday(`polaczone_tabele`.`data`) = 5 GROUP BY `polaczone_tabele`.`id_zespolu``id_zespolu`  ;
COMMIT;
